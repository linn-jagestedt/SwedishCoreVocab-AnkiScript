...
function getHue(text, lastHue) {
	Math.seedrandom(text);
	let randomHue = Math.round(Math.random() * 360);
	while (Math.abs(randomHue - lastHue) < 50 || Math.abs(randomHue - lastHue) > 310)
...
function addListLinks(link) {
	$('li').each(function() {
		var array = $(this).html().split(",");
		for (var i = 0; i < array.length; i++) {
			array[i] = "<a href='" + link + array[i] + "'>" + array[i] + "</a>";
		}
